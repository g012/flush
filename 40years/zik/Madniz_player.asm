...
; - ROM: Check if Madniz_tt_SequenceTable can hold ptrs directly without indexing
;       Madniz_tt_PatternPtrLo/Hi. Can be smaller if not many patterns get repeated
;       (saves table and decode routine)
; - Speed: Inline Madniz_tt_CalcInsIndex
...
; ---------------------------------------------------------------------
    MAC Madniz_TT_FETCH_CURRENT_NOTE
        ; construct ptr to pattern
.constructPatPtr:
        ldy tt_cur_pat_index_c0,x       ; get current pattern (index into Madniz_tt_SequenceTable)
        lda Madniz_tt_SequenceTable,y
    IF TT_USE_GOTO = 1
        bpl .noPatternGoto
...
.noPatternGoto:
    ENDIF
        tay
        lda Madniz_tt_PatternPtrLo,y
        sta tt_ptr
        lda Madniz_tt_PatternPtrHi,y
        sta tt_ptr+1
        ; get new note
    IF TT_USE_OVERLAY = 0
...
.advanceLoop:
    IF TT_USE_OVERLAY = 1
        jsr Madniz_tt_FetchNote
    ELSE
        Madniz_TT_FETCH_CURRENT_NOTE
    ENDIF
...
.pause:
        ...
        ; or commands.
        lda tt_cur_ins_c0,x
        jsr Madniz_tt_CalcInsIndex
        lda Madniz_tt_InsReleaseIndexes-1,y    ; -1 b/c instruments start at #1
...
.newNote:
        ...
        lda Madniz_tt_PercIndexes-TT_FIRST_PERC,y
...
.startInstrument:
    ...
    ENDIF
        ...
        jsr Madniz_tt_CalcInsIndex
        lda Madniz_tt_InsADIndexes-1,y         ; -1 because instruments start at #1
.storeADIndex:
...
.sequencerNextChannel:
    ...
    IF TT_GLOBAL_SPEED = 0
        ...
        ldx tt_cur_pat_index_c0         ; get current pattern (index into Madniz_tt_SequenceTable)
        ldy Madniz_tt_SequenceTable,x          ; Current pattern index now in y
      IF TT_USE_FUNKTEMPO = 0
        lda Madniz_tt_PatternSpeeds,y
        sta tt_timer
      ELSE
        ; Test for odd/even frame
        lda tt_cur_note_index_c0
        lsr
        lda Madniz_tt_PatternSpeeds,y          ; does not affect carry flag
        bcc .evenFrame
...
.storeFunkTempo:
    ...
    ELSE
        ; Global tempo
        ldx #Madniz_TT_SPEED-1
      IF TT_USE_FUNKTEMPO = 1
        lda tt_cur_note_index_c0
        lsr
        bcc .noOddFrame
        ldx #Madniz_TT_ODD_SPEED-1
.noOddFrame:
...
.updateLoop:
        ...
        lda tt_cur_ins_c0,x
    IF Madniz_TT_STARTS_WITH_NOTES = 0
    ...
    ENDIF
        ...
        lda Madniz_tt_PercCtrlVolTable-1,y     ; -1 because values are stored +1
...
.endOfPercussion:
        ...
        sta AUDC0,x     
        ; Set AUDF
        lda Madniz_tt_PercFreqTable-1,y        ; -1 because values are stored +1
    ...
    IF TT_USE_OVERLAY = 1
        ...
        sta tt_cur_ins_c0,x             ; set new instrument
        jsr Madniz_tt_CalcInsIndex
        lda Madniz_tt_InsSustainIndexes-1,y    ; -1 because instruments start at #1
        sta tt_envelope_index_c0,x      
...
Madniz_tt_CalcInsIndex:
        ...
        lsr
        tay
Madniz_tt_Bit6Set:     ; This opcode has bit #6 set, for use with bit instruction
        rts
...
.instrument:
        ...
        lda Madniz_tt_InsCtrlTable-1,y ; -1 because instruments start with #1
        sta AUDC0,x
        ; advance ADSR counter and compare to end of Sustain
        lda tt_envelope_index_c0,x
        cmp Madniz_tt_InsReleaseIndexes-1,y    ; -1 because instruments start with #1
        bne .noEndOfSustain
        ; End of sustain: Go back to start of sustain
        lda Madniz_tt_InsSustainIndexes-1,y    ; -1 because instruments start with #1
.noEndOfSustain:
        tay
        ; Set volume from envelope
        lda Madniz_tt_InsFreqVolTable,y
...
.afterAudioUpdate:
        ...
        echo "Music player size: ", *-Madniz_tt_PlayerStart
