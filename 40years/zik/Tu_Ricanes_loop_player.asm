...
; TIATracker Player
; =====================================================================
Tu_Ricanes_loop_tt_PlayerStart:
...
; - ROM: Check if Tu_Ricanes_loop_tt_SequenceTable can hold ptrs directly without indexing
;       Tu_Ricanes_loop_tt_PatternPtrLo/Hi. Can be smaller if not many patterns get repeated
;       (saves table and decode routine)
; - Speed: Inline Tu_Ricanes_loop_tt_CalcInsIndex
...
; ---------------------------------------------------------------------
    MAC Tu_Ricanes_loop_TT_FETCH_CURRENT_NOTE
        ; construct ptr to pattern
.constructPatPtr:
        ldy tt_cur_pat_index_c0,x       ; get current pattern (index into Tu_Ricanes_loop_tt_SequenceTable)
        lda Tu_Ricanes_loop_tt_SequenceTable,y
    IF TT_USE_GOTO = 1
        bpl .noPatternGoto
...
.noPatternGoto:
    ENDIF
        tay
        lda Tu_Ricanes_loop_tt_PatternPtrLo,y
        sta tt_ptr
        lda Tu_Ricanes_loop_tt_PatternPtrHi,y
        sta tt_ptr+1
        ; get new note
    IF TT_USE_OVERLAY = 0
    ...
    ELSE
        ...
        bit Tu_Ricanes_loop_tt_Bit6Set
.notPrefetched:
        tay
    ENDIF
        lda (tt_ptr),y
...
; ---------------------------------------------------------------------
Tu_Ricanes_loop_tt_Player SUBROUTINE    
        ; ==================== Sequencer ====================
...
.advanceLoop:
    IF TT_USE_OVERLAY = 1
        jsr Tu_Ricanes_loop_tt_FetchNote
    ELSE
        Tu_Ricanes_loop_TT_FETCH_CURRENT_NOTE
    ENDIF
...
.pause:
        ...
        ; or commands.
        lda tt_cur_ins_c0,x
        jsr Tu_Ricanes_loop_tt_CalcInsIndex
        lda Tu_Ricanes_loop_tt_InsReleaseIndexes-1,y    ; -1 b/c instruments start at #1
...
.newNote:
        ...
        lda Tu_Ricanes_loop_tt_PercIndexes-TT_FIRST_PERC,y
...
.startInstrument:
    ...
    ENDIF
        ...
        jsr Tu_Ricanes_loop_tt_CalcInsIndex
        lda Tu_Ricanes_loop_tt_InsADIndexes-1,y         ; -1 because instruments start at #1
.storeADIndex:
...
.sequencerNextChannel:
    ...
    IF TT_GLOBAL_SPEED = 0
        ...
        ldx tt_cur_pat_index_c0         ; get current pattern (index into Tu_Ricanes_loop_tt_SequenceTable)
        ldy Tu_Ricanes_loop_tt_SequenceTable,x          ; Current pattern index now in y
      IF TT_USE_FUNKTEMPO = 0
        lda Tu_Ricanes_loop_tt_PatternSpeeds,y
        sta tt_timer
      ELSE
        ; Test for odd/even frame
        lda tt_cur_note_index_c0
        lsr
        lda Tu_Ricanes_loop_tt_PatternSpeeds,y          ; does not affect carry flag
        bcc .evenFrame
...
.storeFunkTempo:
    ...
    ELSE
        ; Global tempo
        ldx #Tu_Ricanes_loop_TT_SPEED-1
      IF TT_USE_FUNKTEMPO = 1
        lda tt_cur_note_index_c0
        lsr
        bcc .noOddFrame
        ldx #Tu_Ricanes_loop_TT_ODD_SPEED-1
.noOddFrame:
...
.updateLoop:
        ...
        lda tt_cur_ins_c0,x
    IF Tu_Ricanes_loop_TT_STARTS_WITH_NOTES = 0
    ...
    ENDIF
        ...
        lda Tu_Ricanes_loop_tt_PercCtrlVolTable-1,y     ; -1 because values are stored +1
...
.endOfPercussion:
        ...
        sta AUDC0,x     
        ; Set AUDF
        lda Tu_Ricanes_loop_tt_PercFreqTable-1,y        ; -1 because values are stored +1
    ...
    IF TT_USE_OVERLAY = 1
        ...
        sta tt_cur_ins_c0,x             ; set new instrument
        jsr Tu_Ricanes_loop_tt_CalcInsIndex
        lda Tu_Ricanes_loop_tt_InsSustainIndexes-1,y    ; -1 because instruments start at #1
        sta tt_envelope_index_c0,x      
...
; ---------------------------------------------------------------------
Tu_Ricanes_loop_tt_CalcInsIndex:
        ...
        lsr
        tay
Tu_Ricanes_loop_tt_Bit6Set:     ; This opcode has bit #6 set, for use with bit instruction
        rts
...
.instrument:
        ...
        lda Tu_Ricanes_loop_tt_InsCtrlTable-1,y ; -1 because instruments start with #1
        sta AUDC0,x
        ; advance ADSR counter and compare to end of Sustain
        lda tt_envelope_index_c0,x
        cmp Tu_Ricanes_loop_tt_InsReleaseIndexes-1,y    ; -1 because instruments start with #1
        bne .noEndOfSustain
        ; End of sustain: Go back to start of sustain
        lda Tu_Ricanes_loop_tt_InsSustainIndexes-1,y    ; -1 because instruments start with #1
.noEndOfSustain:
        tay
        ; Set volume from envelope
        lda Tu_Ricanes_loop_tt_InsFreqVolTable,y
...
.afterAudioUpdate:
        ...
        echo "Music player size: ", *-Tu_Ricanes_loop_tt_PlayerStart
