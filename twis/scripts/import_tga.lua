...
--	IMPORTANTIEREN(!)
--
-- pour utilser le script d'import de sprite format tga:
-- wine lua.exe import_tga.lua test.tga 64 32 64 32
-- pas oublier d'avoir un répertoire data/
-- Aussi, dans gimp, virer la compression et prendre l'option bottom left
--
--
--
local pathfilename = arg[1]
-- wspr= largeur réelle des sprites
local wspr = tonumber(arg[2])
-- hspr = hauteur réelle des sprites
local hspr = tonumber(arg[3])
-- hwwspr = largeur hardware voulu
local hwwspr = tonumber(arg[4])
-- hwhspr = hauteur hardware voulu
local hwhspr = tonumber(arg[5])
local filename = pathfilename:gsub(".*/","")
local palettefilename = 'data/'..filename:gsub("%.%w+$","_pal.bin")
local spritefilename = 'data/'..filename:gsub("%.%w+$","_sprite.bin")
...
local palfile,sprfile,err

palfile,err = io.open(palettefilename,"wb")
if palfile == nil then print(err) os.exit(false) end
for i=1,cm_len do
  local c = palette[i]
  local entry = math.floor(c.b/8)*1024 + math.floor(c.g/8)*32 + math.floor(c.r/8)
  palfile:write(bytes16(entry));
end
palfile:close()

sprfile,err = io.open(spritefilename,"wb")
if sprfile == nil then print(err) os.exit(false) end
for _,s in ipairs(sprites) do
  for j=1,#s/32 do
    for i=1,32,2 do
      local p1 = s[(j-1)*32+i]
      local p2 = s[(j-1)*32+i+1]
      sprfile:write(string.char(p2*16+p1))
    end
  end
end
sprfile:close()
