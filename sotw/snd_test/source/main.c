...
static void BG_UpdateText(BG* bg)
{
	switch(bg->number)
	{
	case 0:
...
static void TXT_Display(const u16* fnt, c8* str, u32 x, u32 y)
{
    u16* d = &s_bg[0].mapData[y * 32 + x];
    for (; *str;)
        *d++ = fnt[(u32)*str++];
}
...
static void MEM_Fill32(void* mem, u32 v, u32 sz)
{
    u32 *c = mem, *e = c + sz / sizeof(*c);
    //assert(MEM_ALIGN(mem, 4) == mem);
    //assert((sz & 3) == sz);
    for (; c != e; ++c)
        *c++ = v;
}
#endif
static void MEM_Fill8(void* mem, u32 v, u32 sz)
{
    u32 i;
    u8* d = mem;
    u8 x = (u8)v;
...
static void SetPGM1(bool play)
{
    u16 sweepshifts=2,sweepdir=1,sweeptime=7;
    u16 envinit=0xf, envdir=0, envsteptime=0,waveduty=2,soundlength=0;
    u16 loopmode=0,sfreq=0x400;
    ...
    if (play)
    {
        ...
        REG_SOUND1CNT_X = 0x8000+(loopmode<<14)+sfreq;    
    }
    else
    {
        REG_SOUND1CNT_L = 0;
...
typedef u32 x32;
static u16 CXX32_sin[1025];
static x32 CXX32_SinIdx(s32 index)
{
    index &= 0xFFFF;
    if (index < 0x4000)
        return CXX32_sin[index >> 4];
    if (index < 0x8000)
        return CXX32_sin[0x8000 - index >> 4];
    if (index < 0xC000)
        return -CXX32_sin[index - 0x8000 >> 4];
    return -CXX32_sin[(-index & 0xFFFF) >> 4];
}

static u32 s_dsoundBuf[256 / 4];
static void GenDSound(void)
{
    u32 i;
    u8* d = (u8*)s_dsoundBuf;
    for (i = 0; i < sizeof(s_dsoundBuf); ++i)
        *d++ = (u8)(CXX32_SinIdx(i * 0x10000 / 0x100) >> 5);
}
#define REG_DMA1CNT_H   *(u16*)0x40000C6	//DMA1 Control High Value
static void SetDSound(bool play)
{
    if (play)
    {
        REG_SOUNDCNT_H = 0x0b0F;
        REG_DMA1SAD = (u32)s_dsoundBuf;
        REG_DMA1DAD = 0x040000a0;
        REG_DMA1CNT_H = 0xb600;

        REG_TM0CNT_L= 0xFFFF;//0xFBE8;	//16khz playback freq
...
{
    ++s_frameCount;
	REG_IF |= REG_IF; // clear interrupt flag
}
...
__attribute__ ((noreturn)) int main(void)
{
    bool pgm1On = true;
    bool dsoundOn = true;
    const u16* fnt = fnt_euroascii8x8Map;
    ...
    REG_DISPCNT = MODE_0 | BG2_ENABLE;
    {
        ...
        MEM_Fill8(bg, 0, sizeof(*bg));
        bg->number = 2;
        bg->screenBaseBlock = 10;
        ...
        BG_Enable(bg);
        BG_UpdateText(bg);
        dmaCopy(fnt_euroascii8x8Tiles, bg->tileData, fnt_euroascii8x8TilesLen / 2);
        dmaCopy(fnt_euroascii8x8Pal, BG_COLORS, fnt_euroascii8x8PalLen / 2);
    }
    BG_COLORS[15] = RGB8(0, 0, 0);

	REG_SOUNDCNT_X = 0x80; // enable sound
	REG_SOUNDCNT_L = 0x1177; // PGM1 full volume, stereo
	REG_SOUNDCNT_H = 2; // master volume full
    ...
    for (;;)
    {
...

        {
            ...
            TXT_Display(fnt, "[A]    PGM1   [B] DirectSound ", 0, 6);
            if (pgm1On)
                TXT_Display(fnt, "      ON    ", 2, 7);
            else
                TXT_Display(fnt, "      OFF   ", 2, 7);
            if (dsoundOn)
                TXT_Display(fnt, "      ON    ", 16, 7);
            else
                TXT_Display(fnt, "      OFF   ", 16, 7);
        }
    }
}
